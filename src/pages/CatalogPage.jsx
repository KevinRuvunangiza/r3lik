  ...
  return (
    <div className="relative w-full min-h-screen text-[#f4f4f4] flex flex-col selection:bg-[#f4f4f4] selection:text-[#050505]" style={{ backgroundColor: 'var(--color-background)' }}>
      ...
      <main className="p-3 sm:p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-4 md:gap-4 relative z-10 w-full flex-grow font-mono auto-rows-max lg:auto-rows-auto">
        {/* LEFT PANEL: Filters / Categories - Hidden on mobile, visible on lg+ */}
        <div className="hidden lg:flex lg:col-span-2 flex-col gap-4 md:gap-8">
          <div className="border p-3 sm:p-4" style={{ borderColor: 'var(--color-twilight-mauve)', backgroundColor: 'rgba(85, 48, 58, 0.1)' }}>
            <h3 className="text-xs mb-3 sm:mb-4 pb-2 text-[11px] sm:text-xs" style={{ color: 'var(--color-twilight-mauve)', borderBottom: '1px solid', borderColor: 'var(--color-twilight-mauve)' }}>
              SYS_FILTERS
            </h3>
        ...
        <div className="lg:col-span-5 border p-3 sm:p-4 max-h-[60vh] md:max-h-[70vh] lg:max-h-[75vh] overflow-y-auto custom-scrollbar" style={{ borderColor: 'var(--color-twilight-mauve)', backgroundColor: 'rgba(95, 79, 126, 0.1)' }}>
          <div className="flex justify-between items-center mb-4 sm:mb-6 pb-2 text-[11px] sm:text-xs" style={{ borderBottom: '1px solid', borderColor: 'var(--color-twilight-mauve)', color: 'var(--color-twilight-mauve)' }}>
            <h3>
            ...
            {items.map((item) => (
              <Link key={item.id} to={`/catalog/${item.id}`}>
                ...
                <div className="flex justify-between items-start">
                  <span className="text-[9px] sm:text-[10px]" style={{ color: 'var(--color-twilight-mauve)' }}>{item.id}</span>
                  {activeItem.id === item.id && (
                    <TbCrosshair className="animate-spin-slow text-sm" style={{ color: 'var(--color-roan-rouge)' }} />
                  )}
                </div>
                <div>
                  <p className="text-[10px] sm:text-xs mb-1" style={{ color: 'var(--color-twilight-mauve)' }}>{item.sys_name}</p>
                  <h4 className="text-xs sm:text-sm font-bold uppercase truncate" style={{ color: 'var(--color-aster-petal)' }}>
                    {item.title}
                  </h4>
                  <p className="text-[10px] sm:text-xs mt-2" style={{ color: 'var(--color-bottlefly-wings)' }}>{item.price}</p>
                </div>
                </motion.div>
              </Link>
            ))}
        ...
        <div className="lg:col-span-5 border flex flex-col relative overflow-hidden min-h-[50vh] md:min-h-[60vh] lg:min-h-[75vh]" style={{ borderColor: 'var(--color-twilight-mauve)', backgroundColor: 'rgba(95, 79, 126, 0.1)' }}>
          {/* Background ID Text */}
          <div
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                ...
                <div className="absolute top-2 sm:top-4 left-2 sm:left-4 border-l-2 border-t-2 w-3 sm:w-4 h-3 sm:h-4 group-hover:transition-colors group-hover:duration-300" style={{ borderColor: 'var(--color-twilight-mauve)' }}></div>
                <div className="absolute bottom-2 sm:bottom-4 right-2 sm:right-4 border-r-2 border-b-2 w-3 sm:w-4 h-3 sm:h-4 group-hover:transition-colors group-hover:duration-300" style={{ borderColor: 'var(--color-twilight-mauve)' }}></div>
              </Link>

              {/* Data Block */}
              <div className="p-3 sm:p-4 md:p-6 backdrop-blur-sm overflow-y-auto max-h-[40%] lg:max-h-auto" style={{ backgroundColor: 'rgba(30, 24, 33, 0.95)' }}>
                <h2
                ...
                <p className="text-base sm:text-lg mb-3 sm:mb-6" style={{ color: 'var(--color-bottlefly-wings)' }}>{activeItem.price}</p>

                <div className="space-y-1 sm:space-y-2 text-[10px] sm:text-xs mb-3 sm:mb-6 py-2 sm:py-4" style={{ borderTop: '1px solid', borderBottom: '1px solid', borderColor: 'var(--color-roan-rouge)', color: 'var(--color-twilight-mauve)' }}>
